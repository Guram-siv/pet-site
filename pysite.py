...
def log_in():
    ...
    if userpassword is not None:
        ...
        if userpassword[0] == password:
            log = True
            curs.execute(
                "SELECT status FROM persons WHERE mail = %s AND password = %s", (mail, password))
            global person_status
            person_s = curs.fetchone()
            person_status = person_s[0]
            curs.execute(
                "SELECT id FROM persons WHERE mail = %s AND password = %s", (mail, password))
            global person_id
...
\n0) log out\nq) quit the program \n: ")
        ...
        if action == "1":
            ...
            for vet in vets:
                ...
                vet_id = vet[3]
                print(
                    f"Vet name: {vet_name}, Status: {vet_status}, Phone: {vet_phone}, ID: {vet_id}")
            ...
            for pet_owner in pet_owners:
                ...
                print(
                    f"Pet name: {pet_name}, Species: {pet_species}, Breed: {pet_breed}, Owner name: {owner_name}, Owner ID: {owner_id}")

        elif action == "4":
            register_pet(person_id)
            curs.execute(
                "SELECT status FROM persons WHERE id = %s", (person_id, ))
            result = curs.fetchone()
            stat = result[0]
            if stat == "4":
                curs.execute(
                    "UPDATE persons SET status = 5 WHERE id = %s", (person_id, ))
                connection.commit()
            else:
                pass
        elif action == "5":
            ...
            if owner_id is not None:
                ...
                choise = input(
                    "it appears you dont have a pet, do you want to add one? (y/n)")
                if choise == "y":
                    register_pet(person_id)
        elif action == "0":
            exit = False
...
while True:
    ...
    if action == "login":
        ...
        if log == True:
            if person_status == 1:
                print(
                    "Welcome dear User, you registered as a pet owner, your id is %s", (person_id, ))
                ...
                curs.execute(
                    "SELECT vet_id FROM vets WHERE person_id = %s;", (person_id, ))
                result = curs.fetchone()
                vet_id = result[0]
